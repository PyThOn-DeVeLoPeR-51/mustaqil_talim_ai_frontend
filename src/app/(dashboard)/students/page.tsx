...
import type { ResultRead, Student, TaskRead } from "@/types/api";

type StudentFilter = "all" | "active" | "inactive" | "submitted" | "not_submitted";
...
function calcStudentStats(student: Student, tasks: TaskRead[], results: ResultRead[]): StudentStats {
  const assignedTasks = tasks.filter((task) => task.assigned_student_ids?.includes(student.id));
  ...
  return {
    ...
    progress: assignedTasks.length > 0 ? Math.round((submittedTaskIds.size / assignedTasks.length) * 100) : 0,
    lastActivity,
  };
}
...
function groupStudentResultsByTask(studentId: number, results: ResultRead[]) {
  ...
  return [...map.entries()].map(([taskId, rows]) => {
    ...
    const evaluated = sorted.filter((item) => item.status === "evaluated" && typeof item.total_score === "number");
    const bestScore = evaluated.length ? Math.round(Math.max(...evaluated.map((item) => Number(item.total_score)))) : null;
...
function ScoreBadge({ score }: { score?: number | null }) {
  if (typeof score !== "number") return <Badge variant="secondary">Baholanmagan</Badge>;
  if (score >= 86) return <Badge>{Math.round(score)}/100</Badge>;
  if (score >= 60) return <Badge variant="secondary">{Math.round(score)}/100</Badge>;
...
export default function StudentsPage() {
  ...
  const totalStats = useMemo(() => {
    const allStats = students.map((student) => statsByStudent.get(student.id)).filter(Boolean) as StudentStats[];
  ...
  const canCreate = form.full_name.trim().length >= 3;
  ...
  async function addStudent() {
    try {
      ...
      setForm({ full_name: "", university: "", direction: "", stage: "1-bosqich" });
      setCreateOpen(false);
      toast.success("Talaba qo‘shildi. Login/parolni nusxalab talabaga bering.");
    } catch (error) {
      console.error(error);
  ...
  async function toggleActive(student: Student) {
    try {
      const updated = await updateStudent(student.id, { is_active: !student.is_active });
      setStudents((prev) => prev.map((item) => (item.id === updated.id ? updated : item)));
      toast.success(updated.is_active ? "Talaba faollashtirildi." : "Talaba bloklandi.");
    } catch (error) {
      console.error(error);
  ...
  const selectedStats = selectedStudent ? statsByStudent.get(selectedStudent.id) : null;
  const selectedAssignedTasks = selectedStudent
    ? tasks.filter((task) => task.assigned_student_ids?.includes(selectedStudent.id))
    : [];
  const selectedTaskGroups = selectedStudent ? groupStudentResultsByTask(selectedStudent.id, results) : [];
  ...
  return (
    <div className="space-y-5">
            ...
            <Dialog open={createOpen} onOpenChange={setCreateOpen}>
              ...
              <DialogContent className="sm:max-w-lg">
                ...
                <DialogFooter>
                  ...
                  <Button onClick={addStudent} disabled={!canCreate || creating}>
                    {creating ? "Saqlanmoqda..." : "Saqlash va login/parol berish"}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
        ...
        <Card className="border-emerald-200 bg-emerald-50/70">
          <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col gap-2 text-sm md:flex-row">
              <Button variant="outline" onClick={() => copy(createdCredentials.login)}>
                <ClipboardCopy className="mr-2 h-4 w-4" />
                Login: {createdCredentials.login}
              </Button>
              <Button variant="outline" onClick={() => copy(createdCredentials.temporary_password)}>
                <ClipboardCopy className="mr-2 h-4 w-4" />
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryCard
          title="O‘rtacha ball"
          value={totalStats.avgScore !== null ? `${totalStats.avgScore}/100` : "—"}
          hint="Talabalar o‘rtacha natijasi"
          icon={<Award className="h-5 w-5" />}
        />
        <SummaryCard
          title="Faol holat"
          value={totalStats.students ? `${Math.round((totalStats.active / totalStats.students) * 100)}%` : "0%"}
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid gap-2">
              <Label>OTM</Label>
              <Input value={editForm.university} onChange={(event) => setEditForm({ ...editForm, university: event.target.value })} />
            </div>
            <div className="grid gap-2">
              <Label>Yo‘nalishi</Label>
              <Input value={editForm.direction} onChange={(event) => setEditForm({ ...editForm, direction: event.target.value })} />
            </div>
            <div className="grid gap-2">
              <Label>Bosqich</Label>
              <Input value={editForm.stage} onChange={(event) => setEditForm({ ...editForm, stage: event.target.value })} />
            </div>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-h-[88vh] max-w-5xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{selectedStudent?.full_name || "Talaba tafsilotlari"}</DialogTitle>
          </DialogHeader>

          {selectedStudent && (
            <div className="space-y-5">
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <SummaryCard title="Biriktirilgan" value={selectedStats?.assignedTasks || 0} hint="Topshiriqlar soni" icon={<FileText className="h-5 w-5" />} />
                <SummaryCard title="Urinishlar" value={selectedStats?.attempts || 0} hint={`${selectedStats?.completedTasks || 0} task baholangan`} icon={<ClipboardCopy className="h-5 w-5" />} />
                <SummaryCard title="Eng yaxshi ball" value={selectedStats?.bestScore !== null ? `${selectedStats?.bestScore}/100` : "—"} hint="AI baholagan eng yuqori natija" icon={<Award className="h-5 w-5" />} />
                <SummaryCard title="Progress" value={`${selectedStats?.progress || 0}%`} hint="Yuborilgan tasklar ulushi" icon={<CheckCircle2 className="h-5 w-5" />} />
              </div>

              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Login ma’lumotlari</CardTitle>
                </CardHeader>
                <CardContent className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-xl border p-3">
                    <p className="text-xs text-muted-foreground">Login</p>
                    <div className="mt-2 flex items-center justify-between gap-2">
                      <span className="font-mono text-sm">{selectedStudent.login}</span>
                      <Button size="sm" variant="outline" onClick={() => copy(selectedStudent.login)}>
                        Copy
                      </Button>
                    </div>
                  </div>
                  <div className="rounded-xl border p-3">
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2">
                      <Badge variant={selectedStudent.is_active ? "outline" : "destructive"}>
                        {selectedStudent.is_active ? "Faol" : "Bloklangan"}
                      </Badge>
                      <Button size="sm" variant="outline" onClick={() => toggleActive(selectedStudent)}>
                        {selectedStudent.is_active ? "Bloklash" : "Faollashtirish"}
                      </Button>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Biriktirilgan topshiriqlar</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  {selectedAssignedTasks.map((task) => {
                    const rows = results.filter((item) => item.student_id === selectedStudent.id && item.task_id === task.id);
                    const evaluated = rows.filter((item) => item.status === "evaluated" && typeof item.total_score === "number");
                    const best = evaluated.length ? Math.max(...evaluated.map((item) => Number(item.total_score))) : null;
                        ...
                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge variant={task.mode === "etalon" ? "default" : "secondary"}>{modeLabel(task.mode)}</Badge>
                              <Badge variant={task.is_active ? "outline" : "destructive"}>{task.is_active ? "Faol" : "Nofaol"}</Badge>
                            </div>
                            <p className="mt-1 text-xs text-muted-foreground">Task #{task.id} • {rows.length}/2 urinish • Deadline: {formatDate(task.deadline)}</p>
                          </div>
                          <ScoreBadge score={best} />
                        </div>
                        <Progress className="mt-3" value={Math.min(100, Math.round((rows.length / 2) * 100))} />
                      </div>
                    );
                  })}
                    ...
                    <div key={group.taskId} className="rounded-xl border p-3">
                      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">Task #{group.taskId} • {group.attempts}/2 urinish • Oxirgi: {formatDate(group.latest?.created_at)}</p>
                        </div>
                      ...
                      <div className="mt-3 grid gap-2 md:grid-cols-2">
                        {group.rows.map((row) => (
                          <div key={row.id} className="rounded-lg bg-muted/30 p-3 text-sm">
                            <div className="flex items-center justify-between gap-2">
                              <span>{row.attempt_number}-urinish</span>
                              <Badge variant={row.status === "failed" ? "destructive" : row.status === "evaluated" ? "outline" : "secondary"}>
                                {statusLabel(row.status)}
                              </Badge>
                            </div>
                            <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
                              <span>{formatDate(row.created_at)}</span>
                              <span className="font-semibold text-foreground">{scoreText(row.total_score)}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <TableRow key={student.id}>
                      <TableCell>
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <div className="mt-1 text-xs text-muted-foreground">ID #{student.id} • {student.university || "OTM kiritilmagan"}</div>
                            <div className="mt-2">
                              <Badge variant={student.is_active ? "outline" : "destructive"}>{student.is_active ? "Faol" : "Bloklangan"}</Badge>
                            </div>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <Button size="sm" variant="outline" onClick={() => copy(student.login)}>
                            Copy
                          </Button>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">{student.direction || "—"}</div>
                        <div className="mt-1 text-xs text-muted-foreground">{student.stage || "Bosqich kiritilmagan"}</div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm font-medium">{stats?.submittedTasks || 0}/{stats?.assignedTasks || 0} task</div>
                        <div className="mt-1 text-xs text-muted-foreground">{stats?.attempts || 0} urinish, {stats?.evaluated || 0} baholangan</div>
                      </TableCell>
                      <TableCell>
                        <div className="min-w-[150px]">
                          <div className="mb-1 flex items-center justify-between text-xs">
                            <span>{stats?.progress || 0}%</span>
                            <span className="text-muted-foreground">{stats?.completedTasks || 0} tugallangan</span>
                          </div>
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-xs text-muted-foreground">O‘rtacha: {scoreText(stats?.avgScore)}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">{formatDate(stats?.lastActivity)}</div>
                        {(stats?.failed || 0) > 0 && (
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" side="right" sideOffset={8}>
                            ...
                            <DropdownMenuItem onClick={() => toggleActive(student)}>
                              <Lock className="mr-2 h-4 w-4" />
                              {student.is_active ? "Bloklash" : "Faollashtirish"}
                            </DropdownMenuItem>
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  <AlertDialogTitle>Talabani o‘chirmoqchimisiz?</AlertDialogTitle>
                                  <AlertDialogDescription>
                                    Bu amal qaytarilmaydi. Talaba, uning task biriktirishlari va natijalari backend bazasidan o‘chiriladi.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
