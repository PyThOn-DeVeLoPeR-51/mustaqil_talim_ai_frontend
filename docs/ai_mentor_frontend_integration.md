...
4. Yakunlangan diagnostika asosida `/plans/generate` orqali sozlangan LLM (hozir Groq GPT-OSS 120B) reja yaratadi; provider xatosida backend mock fallback qo‘llaydi.
...
```text
10 ta savol → real AI reja yaratish → `generation_source=llm` ni tekshirish → vazifani bajarildi qilish → real AI chatga xabar yuborish → sahifani yangilash
```
